...
export function fetchArtistIfNeeded(userId: number): ThunkResult<any> {
	return (dispatch, getState) => {
		...
		if (!getArtistTracksPlaylistObject(userId.toString())(state)) {
			dispatch(
				getPlaylist(
					SC.getUserTracksUrl(userId),
					getPlaylistName(userId.toString(), PlaylistTypes.ARTIST_TRACKS)
				)
			);
		}
		...
		if (!getArtistLikesPlaylistObject(userId.toString())(state)) {
			dispatch(
				getPlaylist(SC.getUserLikesUrl(userId), getPlaylistName(userId.toString(), PlaylistTypes.ARTIST_LIKES))
			);
		}
	};
}
