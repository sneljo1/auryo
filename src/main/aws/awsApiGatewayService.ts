...
export class AWSApiGatewayService {
    ...
    public async getKeys() {
        try {
            return this.keys = await axios.get<GetKeysResponse>(`${CONFIG.AWS_API_URL}/iot/keys`)
                .then((res) => res.data);
    ...
    public async refresh(refreshToken: string, provider: string) {
        ...
        try {
            ...
            return this.performRequest<TokenResponse>(`/auth/refresh/${provider}`, "POST", { refreshToken })
                .then((res) => res.data);
