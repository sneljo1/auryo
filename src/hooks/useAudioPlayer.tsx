...
export function AudioPlayerProvider({ children, value }: AudioPlayerProviderProps) {
  ...
  useEffect(() => {
    ...
    audioEl.current.addEventListener('loadedmetadata', onMetadataLoaded);
    audioEl.current.addEventListener('canplay', onCanPlay);
    audioEl.current.addEventListener('error', onError);
    audioEl.current.addEventListener('play', onPlay);

    audioEl.current.addEventListener('pause', onPause);
    audioEl.current.addEventListener('ended', onEnded);

    return () => {
      audioEl.current.removeEventListener('loadedmetadata', onMetadataLoaded);
      audioEl.current.removeEventListener('canplay', onCanPlay);
      audioEl.current.removeEventListener('error', onError);
      audioEl.current.removeEventListener('play', onPlay);
      audioEl.current.removeEventListener('pause', onPause);
      audioEl.current.removeEventListener('ended', onEnded);
    };
  }, []);
